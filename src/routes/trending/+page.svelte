	...
	async function fetchSteamLogoFromRawg(game) {
		try {
			...
			const storeResponse = await fetch(
				`https://api.rawg.io/api/games/${game.id}/stores?key=${API_KEY}`
			);
			if (!storeResponse.ok) {
...
<div class="mx-auto p-4">
	...
	<div class="flex justify-center gap-8 flex-wrap py-16">
		{#each games as game}
			<a href="https://rawg.io/games/{game.slug}" alt={game.name} target="_blank" class="card">
				<div class="wrapper">
					<img
				...
				{#if game.steamLogo}
					<img src={game.steamLogo} class="title" alt="Game Logo" />
				{:else}
					<img src="https://via.placeholder.com/300x150?text=No+Logo" class="title" alt="" />
				{/if}
			</a>
		{/each}
	</div>
